...
contract Domains is ERC721URIStorage {
    ...
    // Calculate a price based on the domain length
    function price(string calldata name) public pure returns (uint256) {
        uint256 len = StringUtils.strlen(name);
        require(len > 0, "No name provided");
        if (len == 3) {
            return 5 * 10**17; // 0.5 MATIC (18 decimals)
        } else if (len == 4) {
    ...
    function register(string calldata name) public payable {
        ...
        // Combine the name passed into the fucntion with the TLD
        ...
        console.log(
            "Registering %s.%s on te contract with the tokenID %d",
            name,
            tld,
            newRecordId
        );
